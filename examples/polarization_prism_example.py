...
return_transmission = zp.analyses.transmission(oss, jx=jx, jy=jy, x_phase=x_phase, y_phase=y_phase, sampling="64x64")
...
ret = zp.analyses.polarization_pupil_map(oss, jx=jx, jy=jy, x_phase=x_phase, y_phase=y_phase, sampling="17x17")
df = ret["Data"]["Table"]
...
for ii in range(len(df)):
    ...
    Ey = (
        np.real(df["Ey"].iloc[ii] * np.exp(1j * phi + 1j * df["Phase(Deg)"].iloc[ii] * np.pi / 180)) / xylen
        + df["Py"].iloc[ii]
    )
