...
class TestAbsPath:
    ...
    def test_abspath_raises_file_not_found_error(
        self, tmp_path, input_type, monkeypatch
    ):
        filename = "test.txt"
    ...
    def test_abspath_check_directory_only_does_not_raise_file_not_found_error(
        self, tmp_path, input_type, monkeypatch
    ):
        filename = "test.txt"
        ...
        assert abspath(input_type(filename), check_directory_only=True) == str(
            tmp_path / filename
        )

    def test_abspath_check_directory_only_raises_file_not_found_error(
        self, tmp_path, input_type, monkeypatch
    ):
...
class TestNumberToStringConversion:
    ...
    def test_xtoa_converts_number_correctly(
        self, number, expected_output, decimal_point, thousands_separator
    ):
        result = xtoa(
            number, decimal_point=decimal_point, thousands_separator=thousands_separator
        )
    ...
    def test_xtoa_works_correctly_for_opticstudio(self, simple_system):
        ...
        analysis = new_analysis(
            simple_system, constants.Analysis.AnalysisIDM.FftThroughFocusMtf
        )
        ...
        analysis_settings.ModifySettings(
            cfgoutfile, "TFM_DELTAFOC", xtoa(deltafocus, thousands_separator=None)
        )
        analysis_settings.ModifySettings(
            cfgoutfile, "TFM_STEPS", xtoa(steps, thousands_separator=None)
        )
...
class TestStringToNumberConversion:
    ...
    def test_atox_converts_string_correctly(
        self, string, expected_output, dtype, decimal_point, thousands_separator
    ):
        result = atox(
            string,
            dtype=dtype,
    ...
    def test_atox_raises_error_correctly(
        self, string, dtype, thousands_separator, decimal_point
    ):
    ...
    def test_atox_returns_correct_results_for_opticstudio(self, simple_system):
        ...
        analysis = new_analysis(
            simple_system, constants.Analysis.AnalysisIDM.PolarizationPupilMap
        )
        ...
        analysis_settings.ModifySettings(
            cfgoutfile, "PPM_PX", xtoa(xphase, thousands_separator=None)
        )
        if isinstance(surface, str):
            analysis_settings.ModifySettings(cfgoutfile, "PPM_SURFACE", surface)
        else:
            analysis_settings.ModifySettings(
                cfgoutfile, "PPM_SURFACE", xtoa(surface, thousands_separator=None)
            )
